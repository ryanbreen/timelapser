...
const collectAllFiles = async (record) => {
    ...
    console.log("Found %s files between %s and %s", files.length, new Date(record.start), new Date(record.end))
...
const callFFMPEG = async (frames, name) => {
    ...
    const produce = spawn('ffmpeg', ['-r','60','-pattern_type','glob','-i','*.png',`../processed/${name}.mp4`], { cwd: '/pool/view_data/temp/' })
