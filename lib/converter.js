...
//    const produce = spawn('ffmpeg', ['-r','60','-pattern_type','glob','-i','*.png', '-c:v', 'libx265', '-preset', 'slower', '-crf', '24', '-pix_fmt', 'yuv420p10le', `../processed/${name}.mp4`], { cwd: '/pool/view_data/temp/' })
    const produce = spawn('ffmpeg', ['-r',fps ? fps : '120','-pattern_type','glob','-i','*.png', '-c:v', 'libx265', '-preset', 'slow', '-crf', '26', '-pix_fmt', 'yuv420p10le', `../processed/${name}.mp4`], { cwd: '/pool/view_data/temp/' })
